...
def SDElikelihood(key, y, param, dt, P, obs_interval):
    ...
    with loops.Scope() as s:
        ...
        for i in s.range(num_steps):      
            ...
            s.temp_state, s.temp_time = EulerMaruyama(euler_subkey, param[:3], s.zz, s.temp_time, dt, P, N)  
            s.zz = s.temp_state[:,-1,:]
...
class LogPosterior(object):
    ...
    def _transform_from_constraint(self, untransformed_parameters):
        ...
        tx_x0  = Utx_THETA[3]
        tx_sigm = np.log(Utx_THETA[4])
        return np.array([tx_beta1, tx_beta2, tx_beta3,\
            tx_x0, tx_sigm])
