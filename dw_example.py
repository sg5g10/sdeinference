...
def mcmc_runner(init, logprob, iters):
        ...
        return MCMCsampler.run(random.PRNGKey(1),init)[0]

def numpyro_model(data=None, solver=None, num_bases=50, solver_times=None):
...
def main():
        ...
        trace, sample_path_pm = MCMCsampler.run(random.PRNGKey(10), X0)
        
        pmmh_trace = trace[args.pmmh_warmup:,:]
        pmmh_trace_thin = pmmh_trace[::args.pmmh_thin,:]
        print('PMMH estimates: ', pmmh_trace_thin.mean(axis=0))
        
        
        # Now run VI with SA-ODE with KL expansion
        n = args.num_qsamples
        times = jnp.arange(0,10,.1)
        saode = SAODE_DW(args.end_point, num_bases=args.num_bases)
        vb_ppc_samples, sample_path_vb = run_vi(Y, numpyro_model, saode, random.PRNGKey(10), \
        step_size=1e-2, num_bases = args.num_bases, iterations=args.vi_iters, \
        ppc_n=args.num_qsamples, solver_times=times, batchsize=50)
        ...
        print('VI estimates: ', vb_params.mean(axis=0))
